...
and local (LocalLLMProvider) backends based on ConfigurationService configuration.
...
class LLMBackendRouter:
    ...
    The backend selection is read from ``ConfigurationService.get_llm_backend()``
    ...
    def translate(self, text: str, **kwargs) -> tuple:
        ...
        Reads ``llm_backend`` from ConfigurationService on every call.
