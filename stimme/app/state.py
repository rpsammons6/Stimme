...
class AppState:
    ...
    def to_json(self) -> dict:
        """Serialize recoverable fields to a JSON-serializable dict.

        Includes: input_text, translation_tabs (source_preview, source_full,
        translation, commentary, pdf_path per tab), active_translation_index,
        pdf_file, pdf_path, center_tab, book_mode.

        datetime values are converted to ISO 8601 strings.
        Sets version to 1 and saved_at to current ISO 8601 timestamp.
        """
        serialized_tabs = []
        for tab in self.translation_tabs:
            serialized_tab: Dict[str, Any] = {}
            for key in ("source_preview", "source_full", "translation", "commentary", "pdf_path"):
                value = tab.get(key)
                if isinstance(value, datetime):
                    serialized_tab[key] = value.isoformat()
                else:
                    serialized_tab[key] = value
        ...
        return {
            ...
            "translation_tabs": serialized_tabs,
            "active_translation_index": self.active_translation_index,
            "pdf_file": self.pdf_file,
            "pdf_path": self.pdf_path,
            "center_tab": self.center_tab,
            "book_mode": self.book_mode,
        }

    @classmethod
    def from_json(cls, data: dict) -> "AppState":
        """Create an AppState from a serialized dict.

        Missing keys use default values (empty string, None, False, etc.).
        Unknown keys are silently ignored.
        Restores translation_tabs as list of dicts (no datetime parsing).
        """
        state = cls()
        state.input_text = data.get("input_text", "")
        state.active_translation_index = data.get("active_translation_index", -1)
        state.pdf_file = data.get("pdf_file", None)
        state.pdf_path = data.get("pdf_path", None)
        state.center_tab = data.get("center_tab", 0)
        state.book_mode = data.get("book_mode", False)

        raw_tabs = data.get("translation_tabs", [])
        restored_tabs: List[Dict[str, Any]] = []
        for tab in raw_tabs:
            restored_tabs.append({
                "source_preview": tab.get("source_preview"),
                "source_full": tab.get("source_full"),
                "translation": tab.get("translation"),
                "commentary": tab.get("commentary"),
                "pdf_path": tab.get("pdf_path"),
            })
        state.translation_tabs = restored_tabs
